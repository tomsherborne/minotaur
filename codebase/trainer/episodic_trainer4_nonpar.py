...
from codebase.trainer import DivergenceKernel

logger = logging.getLogger(__name__)


@Trainer.register("episodic_trainer_v4", constructor="from_partial_objects")
class EpisodicTrainerV4(GradientDescentTrainer):
    """
    ...
    V2: we use Registrable divergence Kernels. V3 is redundant
    V4: the batches for outer loop are *non-parallel*. 
    ...
    def outer_loop(self,
        ...
        # logger.info(f"K={self._inner_steps} elapsed. Running outer-loop update with outer-batch")

        batch_loss = 0.0
        batch_reg_loss = None if self.model.get_regularization_penalty() is None else 0.0
        batch_group_outputs = []

        for split_batch_group in outer_loop_batch_group:

            if len(split_batch_group)!=2:
                logger.info(f"Skipping Outer Loop due to length of split_batch_group being: {len(split_batch_group)}!=2")
                break
            
            source_batch, target_batch = split_batch_group
            ...
            with amp.autocast(self._use_amp):
                ...
                source_outputs = self.batch_outputs(source_batch, for_training=True, return_encoder_states=True)
                target_outputs = self.batch_outputs(target_batch, for_training=True, return_encoder_states=True, use_outer_tokens=True)
                ...
                if source_reg_loss is not None: # Not used
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        logger.info(f"EpisodicTrainer V4 with outer->{self._use_outer_loop} div_kernel->{self._divergence_kernel} *************")
        ...
        inner_loop_generator =  common_util.lazy_groups_of(batch_group_generator, self._inner_steps)
        
        ## Episodic V4 groups batches into pairs for source and target throughput (non-parallelism in the divergence kernel)
        ...
        outer_batch_group_generator = common_util.lazy_groups_of(
            outer_batch_generator, group_size=2
        )

        # Iterable[Iterable[Iterator[TensorDict]]]
        outer_batch_group_generator = common_util.lazy_groups_of(
            outer_batch_group_generator, self._num_gradient_accumulation_steps
        )
